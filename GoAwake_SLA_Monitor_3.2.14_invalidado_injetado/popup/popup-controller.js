  ...
  if (btnDashboard) {
    btnDashboard.addEventListener('click', async function() {
      try {
        ...
        var preflightResults = await chrome.scripting.executeScript({
          target: { tabId: tab.id },
          func: function () {
            var required = [
              'GoAwakeProductivityConfig',
              'GoAwakeAuditUtils',
              'GoAwakeDashboardPeriod',
              'GoAwakeAuditNormalizer',
              'GoAwakeIntegrityValidator',
              'GoAwakeDashboardEngine',
              'GoAwakeDashboardPdf',
              'GoAwakeDashboardView'
            ];
            var missing = required.filter(function (name) {
              return !window[name];
            });
            if (window.GoAwakeDashboardPdf &&
                typeof window.GoAwakeDashboardPdf.downloadBuilt !== 'function') {
              missing.push('GoAwakeDashboardPdf.downloadBuilt');
            }
            if (window.GoAwakeDashboardView &&
                typeof window.GoAwakeDashboardView.showPdfReport !== 'function') {
              missing.push('GoAwakeDashboardView.showPdfReport');
            }
            return {
              ok: missing.length === 0,
              missing: missing,
        ...
        var preflight = preflightResults[0] && preflightResults[0].result;
        if (!preflight || preflight.ok !== true) {
          throw new Error(
            'Módulos do Dashboard não carregados: ' +
            ((preflight && preflight.missing || []).join(', ') || 'falha desconhecida')
          );
        }

        await chrome.scripting.executeScript({
          target: { tabId: tab.id },
          files: ['content/hidden-dashboard-runtime.js']
        });
        ...
        atualizarStatus('Dashboard: ' + detail, 'stopped');
        alert('Não foi possível abrir o Dashboard.\n\n' + detail);
