(function (global) {
  ...
  function createFilename(period) {
    ...
    var stamp = [
      ...
      String(end.getMinutes()).padStart(2, '0');
    return 'Relatorio_Gerencial_Produtividade_Moby_' +
  ...
  function createPage(sectionTitle, period, compactHeader) {
    ...
    if (!compactHeader) {
      ...
      page.commands.push(textColorCommand(
        MARGIN + 23, headerY + 24, 12, 'DT', true, 1, 1, 1
      ));
      page.commands.push(textColorCommand(
        MARGIN + 61,
        headerY + 33,
        16,
        sectionTitle || 'Relatório Gerencial de Produtividade Operacional',
        true,
        1,
        1,
        1
      ...
      page.commands.push(textColorCommand(
        ...
        headerY + 15,
        7.8,
        'Goawake SLA Monitor · Inteligência operacional por autor',
        false,
        0.88,
        0.93,
        1
  ...
  function buildPages(data, period, collectionMeta) {
    ...
    var summary = createPage(
      'Relatório Gerencial de Produtividade Operacional',
      period,
      false
    );
    pages.push(summary);
    ...
    summary.commands.push(strokeRoundedRectRgbCommand(
      MARGIN,
      42,
      PAGE_WIDTH - MARGIN * 2,
      130,
    ...
    var notes = [
      'Auditorias consolidadas: cada ID é contado uma vez, considerando a tratativa mais recente.',
      'Duração: diferença entre “Disponível em” e “Tratado em”; o tempo médio usa somente registros com os dois horários reconhecidos.',
      'Faixas de produtividade: até 2 minutos; acima de 2 e até 5 minutos; acima de 5 minutos.',
      'Sem duração calculável: horário ausente, inválido ou incompatível. Integridade dos dados: ' +
        percent(data.summary.qualityScore) + '.'
    ];
    ...
    var charts = createPage(
      'Produção por hora e ranking operacional',
      period,
      true
    );
    pages.push(charts);
  ...
  function buildPdf(data, period, collectionMeta) {
    var pageStreams = buildPages(data, period, collectionMeta);
  ...
  function download(data, period, collectionMeta) {
    var result = buildPdf(data, period, collectionMeta);
