(function (global) {
  ...
  function showPdfReport(pdfResult, period, collectionMeta) {
    var previous = document.getElementById('goawake-dashboard-root');
    if (previous) {
      if (typeof previous.__goAwakeCleanup === 'function') {
        previous.__goAwakeCleanup();
      }
      previous.remove();
    }
    if (!pdfResult || !(pdfResult.bytes instanceof Uint8Array)) {
      throw new Error('A prévia recebeu um PDF inválido.');
    }

    var blob = new Blob([pdfResult.bytes], { type: 'application/pdf' });
    var objectUrl = URL.createObjectURL(blob);
    var overlay = document.createElement('div');
    overlay.id = 'goawake-dashboard-root';
    overlay.className = 'goawake-dashboard-root goawake-pdf-preview-root';
    overlay.innerHTML = `
      <div class="goawake-pdf-preview-shell">
        <header class="goawake-pdf-preview-toolbar">
          <div>
            <strong>Relatório Executivo de Produtividade Operacional</strong>
            <span>Modelo SLA Base M1 · Build 3.2.14-R2 ·
              ${utils.escapeHtml(formatPeriodDateTime(period.start))} a
              ${utils.escapeHtml(formatPeriodDateTime(period.end))}</span>
          </div>
          <div class="goawake-pdf-preview-actions">
            <button id="gd-pdf-download">Baixar este PDF</button>
            <button id="gd-pdf-close" class="close" aria-label="Fechar">×</button>
          </div>
        </header>
        <iframe
          id="gd-pdf-frame"
        ...
        ></iframe>
        <p class="goawake-pdf-preview-note">
          A visualização e o arquivo baixado utilizam exatamente o mesmo documento.
          ${collectionMeta && collectionMeta.partialCompletion
            ? 'A coleta foi finalizada com os dados disponíveis antes da falha de rede.'
            : ''}
        </p>
      </div>
    `;
    overlay.__goAwakeCleanup = function () {
      if (objectUrl) {
    ...
    document.body.appendChild(overlay);
    overlay.querySelector('#gd-pdf-frame').src = objectUrl;
    overlay.querySelector('#gd-pdf-download').addEventListener('click', function () {
      global.GoAwakeDashboardPdf.downloadBuilt(pdfResult);
    });
    overlay.querySelector('#gd-pdf-close').addEventListener('click', function () {
      overlay.__goAwakeCleanup();
      overlay.remove();
    });
    return {
      pdfResult: pdfResult,
