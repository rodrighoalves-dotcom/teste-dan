...
const DASHBOARD_STALLED_COLLECTION_MS = 45000;
...
async function prepararColetaParaDashboard() {
  if (!authorCollectionRunning) return;

  const deadline = Date.now() + 12000;
  let closeRequested = false;
  while (authorCollectionRunning && Date.now() < deadline) {
    if (dedicatedAuditPurpose === 'dashboard') {
      throw new Error('Já existe um Dashboard sendo gerado. Aguarde a conclusão.');
    }

    if (dedicatedAuditPurpose === 'author-popup' &&
        Number.isInteger(dedicatedAuditTabId) &&
        !closeRequested) {
      closeRequested = true;
      try {
        await chrome.tabs.remove(dedicatedAuditTabId);
      } catch (error) {
        // A coleta pode ter encerrado a guia entre a verificação e o fechamento.
      }
    }
  ...
  if (Number.isInteger(dedicatedAuditTabId)) {
    try {
      await chrome.tabs.get(dedicatedAuditTabId);
    } catch (error) {
  ...
  throw new Error(
    'A coleta automática por autor ainda está encerrando. Aguarde alguns segundos e tente novamente.'
  );
}
...
async function recuperarResultadoParcialDashboard(auditTabId) {
  try {
    const results = await chrome.scripting.executeScript({
      target: { tabId: auditTabId },
      func: function () {
        if (!window.GoAwakeAutomaticAuditCollector ||
            typeof window.GoAwakeAutomaticAuditCollector.getPartialResult !== 'function') {
          return null;
        }
        return window.GoAwakeAutomaticAuditCollector.getPartialResult(
          'recovered-after-network-inactivity'
        );
      }
    });
    return results[0] && results[0].result ? results[0].result : null;
  } catch (error) {
    console.warn('[SLA][Dashboard] Resultado parcial indisponível:', error);
    return null;
  }
}

async function executarColetaDashboardResiliente(auditTabId, period) {
  ...
  const execution = chrome.scripting.executeScript({
    ...
    status: 'completed',
    result: results[0] && results[0].result
  })).catch((error) => ({
    status: 'failed',
    error: error
  }));
  ...
  while (true) {
    const outcome = await Promise.race([
      ...
      new Promise((resolve) => setTimeout(
        () => resolve({ status: 'checking-inactivity' }),
        1000
      ))
    ]);
    ...
    if (Date.now() - dashboardLastProgressAt < DASHBOARD_STALLED_COLLECTION_MS) {
      continue;
    }

    await enviarProgressoDashboard({
      message: 'A Audit ficou sem resposta. Finalizando com os dados já coletados...'
    });
    const partial = await recuperarResultadoParcialDashboard(auditTabId);
    if (partial && Array.isArray(partial.records)) {
      partial.partialCompletion = true;
      partial.completionReason = 'recovered-after-network-inactivity';
      return partial;
    }
    throw new Error(
      'A página Audit ficou sem responder e ainda não havia dados consistentes para finalizar o relatório.'
    );
  }
}

async function executarDashboardAuditOculto(message, fatigueTab) {
  await prepararColetaParaDashboard();
  authorCollectionRunning = true;
  try {
    if (!message.period ||
    ...
    await enviarProgressoDashboard({
      message: 'Preparando a guia Audit para a consulta...'
    });
    ...
    await esperarGuiaCarregar(auditTab.id, 45000);
    await enviarProgressoDashboard({
      message: 'Audit carregada. Aplicando o período selecionado...'
    });
    ...
    await enviarProgressoDashboard({
      message: 'Período confirmado. Coletando as páginas da Audit...'
    });
    const result = await executarColetaDashboardResiliente(
      auditTab.id,
      message.period
    );
    ...
    authorCollectionRunning = false;
    dashboardLastProgressAt = 0;
  }
}

async function enviarProgressoDashboard(progress) {
  if (!Number.isInteger(dashboardProgressTargetTabId)) return;
  try {
...
async function entregarResultadoDashboard(fatigueTabId, response, responseType) {
  var usePdfModelV2 = responseType === 'pdf-model-v2';
  var payload = response && response.ok === true
    ? {
        type: usePdfModelV2
          ? 'slaHiddenDashboardResultV2'
          : 'slaHiddenDashboardResult',
        result: response.result,
        period: response.period
      }
    : {
        type: usePdfModelV2
          ? 'slaHiddenDashboardErrorV2'
          : 'slaHiddenDashboardError',
...
async function encaminharProgressoDashboard(message, senderTabId) {
  ...
  dashboardLastProgressAt = Date.now();
  await enviarProgressoDashboard(message.progress || {});
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'runHiddenAuditDashboard') {
    ...
    executarDashboardAuditOculto(message, sender.tab)
      .then((response) => entregarResultadoDashboard(
        fatigueTabId,
        response,
